...
var FeedListController = function($rootScope, $scope, FeedService, InstagramService, $route, $routeParams, $location, data, app, appName, $sce, $q, ngMaterial) {
    ...
    if($routeParams.hasOwnProperty('sponsor')){
        $scope.feedItemScrollAmount = $scope.sponsorItems.length;
    }
    ...
    $scope.onScroll = function() {
        ...
        } else if (articles.length < $scope.postsPerPage) {
            ...
            if (Number($scope.appConfig.adsPerPage) > 0 && $scope.appConfig.displayAds === 'true') {
                ...
                if ($scope.currentView === 'category') {
                    ...
                    if(index === 2){
                        ...
                        tbAdItem.tbAdId = 'taboola-below-article-'+ $scope.paged;
    ...
    $scope.createTaboolaContainer = function(pageNum){
        ...
        var taboola_container_id = 'taboola-below-article-' + pageNum;
    ...
    $scope.getNext = function(params) {
        ...
        $scope.clearAds().then(function() {
            ...
            if ($routeParams.page !== 'hottest' && $routeParams.page !== 'best' && $routeParams.page !== 'latest') {
                ...
                if ($scope.currentView !== 'search' && $scope.currentView !== 'sponsor') {
        ...
        if ($scope.currentView === 'list' || $scope.currentView === 'search' || $scope.currentView === 'category') {
            ...
            if ($scope.posts.length === 0) {
                ...
                angular.forEach($scope.posts, function(item, index) {
                    ...
                    if ($scope.sponsors !== null && $scope.sponsorItems.length > 0) {
                        if (index > 0 && index % 2 === 0) {
                            console.log('pushing sponsor post: ', $scope.sponsorItems[$scope.sponsorCount]);
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        if (($scope.sponsors === null || $scope.sponsors.length > $scope.postsPerPage) || $scope.currentView === 'search' || $scope.currentView === 'list' || $scope.currentView === 'post' && $scope.currentView !== 'ads') {
